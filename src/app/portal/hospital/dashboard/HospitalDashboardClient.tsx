...
import AgreementBanner from "@/components/portal/AgreementBanner";
...
export default function HospitalDashboardClient({ userFullName, userRole, hospitalId, hospitalName, hasActiveAgreement = false, pendingAgreement = null }: Props) {
  ...
  return (
    <>
      ...
      {/* Agreement banner — shown when a new agreement is awaiting review */}
      {pendingAgreement && (
        <div className="max-w-5xl mx-auto">
          <AgreementBanner
            agreement={pendingAgreement}
            onAccepted={() => { /* router.refresh() handled inside */ }}
          />
        </div>
      )}

      {/* EasyHeals Network status strip */}
      {hasActiveAgreement && (
        <div className="max-w-5xl mx-auto px-4 pt-2">
          <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-2">
            <span className="text-sm">🌐</span>
            <p className="text-xs font-semibold text-emerald-700 flex-1">EasyHeals Network Partner</p>
            <Link href="/portal/referrals" className="text-xs font-semibold text-emerald-600 hover:text-emerald-800 transition">
              Referrals →
            </Link>
            <Link href="/portal/earnings" className="text-xs font-semibold text-emerald-600 hover:text-emerald-800 transition ml-3">
              Earnings →
            </Link>
          </div>
        </div>
          ...
          {navItems.map((n) => (
            ...
            >
              ...
              {n.badge !== null && n.badge !== undefined && n.badge > 0 && (
                <span className={`w-5 h-5 rounded-full text-[10px] font-bold flex items-center justify-center ${activeTab === n.tab ? "bg-white/20 text-white" : "bg-amber-500 text-white"}`}>
                  {n.badge}
                </span>
              )}
            </button>
