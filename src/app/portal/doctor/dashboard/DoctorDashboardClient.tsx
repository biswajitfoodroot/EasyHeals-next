...
export default function DoctorDashboardClient({ userFullName, userRole, doctorId }: Props) {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        const res = await fetch(`/api/v1/portal/appointments?${params}`, { credentials: "include" });
  ...
  const stats = [
    ...
    { label: "Upcoming",  value: upcoming.length,   icon: "⏳", hi: false },
    ...
    <div className="min-h-screen bg-slate-50 flex">
      ...
      <aside className="w-14 lg:w-56 bg-white border-r border-slate-200 flex flex-col shrink-0 sticky top-0 h-screen">
        ...
        <nav className="flex-1 p-2 space-y-1">
          {[
            { href: "/portal/doctor/dashboard", icon: "🏠", label: "Dashboard",    active: true },
            { href: "/portal/doctor",           icon: "👤", label: "Edit Profile", active: false },
            { href: "/portal/dashboard",        icon: "📅", label: "Appointments", active: false },
          ].map((n) => (
            <Link
              key={n.label}
              href={n.href}
      ...
      <main className="flex-1 min-w-0 overflow-y-auto">
        <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {stats.map((s) =>
              loading ? (
                ...
                <div
                  key={s.label}
                  className={`bg-white rounded-xl border p-4 shadow-sm ${s.hi ? "border-green-300 bg-green-50" : "border-slate-200"}`}
                >
                  <div className="text-2xl mb-1">{s.icon}</div>
                  <div className={`text-3xl font-bold ${s.hi ? "text-green-700" : "text-slate-800"}`}>{s.value}</div>
                      ...
                      <div className="flex flex-col gap-2 items-end shrink-0">
                        {isPending && (
                          <>
                            ...
                            <button
                              onClick={() => act(appt.id, "reject", "Doctor unavailable")}
                              disabled={busy}
                        ...
                        {isConfirmed && (
                          <>
                            {isOnline && (
                              <Link
                                href={`/portal/consultation/start?appointmentId=${appt.id}`}
                            ...
                            <button
                              ...
                              disabled={busy}
                              className="px-4 py-2 text-sm font-semibold text-slate-700 border border-slate-200 rounded-xl hover:bg-slate-50 disabled:opacity-50 transition"
                            >
                              Mark Complete
                            </button>
                          </>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
            {[
              { href: "/portal/doctor",     icon: "✏️", label: "Edit Profile",       desc: "Update bio, fees, qualifications" },
              { href: "/portal/dashboard",  icon: "📊", label: "Provider Dashboard", desc: "Appointment tools & consultation" },
              { href: "/hospitals",         icon: "🔍", label: "Browse Hospitals",   desc: "View hospital listings" },
            ].map((l) => (
              <Link
                key={l.label}
                href={l.href}
