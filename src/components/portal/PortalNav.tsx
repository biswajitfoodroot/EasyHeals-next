...
export default function PortalNav({ userRole, userName, entityName, hasActiveAgreement = false }: Props) {
  ...
  return (
    <aside className="w-60 bg-white border-r border-slate-200 flex flex-col shrink-0 h-screen shadow-sm overflow-y-auto">
      {/* Brand */}
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {item.badge && (
                <span className="flex ml-auto text-[10px] bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded-full font-semibold">
                  {item.badge}
                </span>
              )}
            </Link>
      ...
      <div className="p-3 border-t border-slate-100 shrink-0">
        ...
        <button
          ...
          className="mt-2 text-xs text-slate-400 hover:text-red-500 transition w-full text-left"
        >
          Sign out →
        </button>
      </div>
